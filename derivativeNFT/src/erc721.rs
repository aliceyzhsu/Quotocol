...
sol! {
    ...
    event URISet(uint256 indexed token_id, string uri);
    event AuthoritySet(address indexed old_auth, address indexed new_auth);
    event SrcContractSet(address indexed src_contract_addr);
    ...
    error NoURITokenId(uint256 token_id);
    error NotAuthority(address from);
}
